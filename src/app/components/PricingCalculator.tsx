            ...
            <CommandGroup heading="Inventory and recent materials">
              {suggestions.map((suggestion) => (
                ...
                >
                  ...
                  {suggestion.available && <span className="text-[9px] text-muted-foreground">{suggestion.available}</span>}
                </CommandItem>
              ))}
            </CommandGroup>
            <CommandGroup>
...
function MoneyInput({ valueCentavos, onChange, label }: {
  ...
  return (
    <label className="space-y-1">
      <span className="text-[8px] uppercase tracking-[0.16em] text-muted-foreground">{label}</span>
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-[9px] uppercase tracking-[0.22em] text-muted-foreground">Custom pricing</p>
        ...
        <div className="flex flex-wrap gap-2">
          <label className="flex min-h-11 cursor-pointer items-center border border-border bg-card px-3 text-[10px] uppercase tracking-wider text-muted-foreground">
          ...
          <button type="button" onClick={saveVersion} className="min-h-11 border border-border bg-card px-3 text-[10px] uppercase tracking-wider text-foreground"><History className="mr-2 inline" size={13} />Save version</button>
      ...
      {trackerBlocks.length > 0 && (
        <section className="border border-amber-200 bg-amber-50 p-4" aria-label="CSV import review">
          <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-medium text-amber-950">Review {trackerBlocks.length} pricing blocks</p><p className="mt-1 text-[9px] text-amber-800">Nothing is imported until you choose a block. Warning rows require manual verification.</p></div><button type="button" onClick={() => setTrackerBlocks([])} className="min-h-11 px-3 text-[10px] text-amber-900">Cancel</button></div>
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {trackerBlocks.map((block) => (
              <article key={block.id} className="border border-amber-200 bg-card p-3">
                <div className="flex justify-between gap-3"><div><p className="text-[11px] font-medium">{block.name}</p><p className="mt-1 text-[9px] text-muted-foreground">{block.lines.length} staged lines · {block.warnings.length} warnings</p></div><button type="button" onClick={() => loadTrackerBlock(block)} className="min-h-11 border border-foreground px-3 text-[9px] uppercase tracking-wider">Load reviewed block</button></div>
                {block.warnings.slice(0, 3).map((warning) => <p key={warning} className="mt-2 text-[9px] text-amber-800">• {warning}</p>)}
              </article>
            ))}
          </div>
        </section>
      ...
      {versions.length > 0 && (
        <details className="border border-border bg-card p-3">
          <summary className="cursor-pointer text-[9px] uppercase tracking-[0.18em] text-muted-foreground">Saved pricing history ({versions.length})</summary>
          <div className="mt-3 space-y-2">{versions.slice().reverse().slice(0, 8).map((version) => <div key={version.id} className="flex justify-between border-t border-border pt-2 text-[10px]"><span>{version.pieceName} · v{version.version}</span><span className="text-muted-foreground">{new Date(version.createdAt).toLocaleString("en-PH")}</span></div>)}</div>
        </details>
      )}

      <section className="grid grid-cols-1 gap-3 border border-border bg-card p-4 sm:grid-cols-3">
        <label className="space-y-1">
          <span className="text-[8px] uppercase tracking-[0.16em] text-muted-foreground">Project</span>
        ...
        <label className="space-y-1">
          <span className="text-[8px] uppercase tracking-[0.16em] text-muted-foreground">Client</span>
        ...
        <label className="space-y-1">
          <span className="text-[8px] uppercase tracking-[0.16em] text-muted-foreground">Piece or service</span>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1.6fr)_minmax(280px,0.8fr)]">
        <section className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[9px] uppercase tracking-[0.2em] text-muted-foreground">Cost lines</p>
              <p className="mt-0.5 text-[9px] text-muted-foreground">Quantity × unit cost is calculated automatically.</p>
            </div>
            <span className="font-mono text-[10px] text-muted-foreground">{lines.length} lines</span>
          </div>
            ...
            {lines.map((line) => (
              <article key={line.id} className="border border-border bg-card p-3">
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-[minmax(150px,1fr)_110px_90px_120px_30px] sm:items-end">
                  <label className="space-y-1">
                    <span className="text-[8px] uppercase tracking-[0.16em] text-muted-foreground">Description</span>
                  ...
                  <label className="space-y-1">
                    <span className="text-[8px] uppercase tracking-[0.16em] text-muted-foreground">Type</span>
                    <select value={line.category} onChange={(event) => updateLine(line.id, { category: event.target.value as PricingLineCategory })} className="min-h-10 w-full border border-border bg-card px-2 text-[10px] outline-none">
                      {Object.entries(CATEGORY_LABEL).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
                    </select>
                  </label>
                  <label className="space-y-1">
                    <span className="text-[8px] uppercase tracking-[0.16em] text-muted-foreground">Qty / hours</span>
                ...
                <div className="mt-2 flex items-center justify-between border-t border-border pt-2">
                  <span className="text-[8px] uppercase tracking-[0.14em] text-muted-foreground">{CATEGORY_LABEL[line.category]}</span>
        ...
        <aside className="space-y-3 xl:sticky xl:top-0 xl:self-start">
          <section className="border border-border bg-card p-4">
            <div className="mb-4 flex items-center gap-2">
              ...
              <p className="text-[9px] uppercase tracking-[0.2em] text-muted-foreground">Commercial adjustments</p>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setMarkupType("fixed")} className={`min-h-10 border text-[10px] ${markupType === "fixed" ? "border-foreground bg-foreground text-card" : "border-border text-muted-foreground"}`}>Fixed markup</button>
              <button type="button" onClick={() => setMarkupType("percentage")} className={`min-h-10 border text-[10px] ${markupType === "percentage" ? "border-foreground bg-foreground text-card" : "border-border text-muted-foreground"}`}>Percentage</button>
            </div>
            <label className="mt-3 block space-y-1">
              <span className="text-[8px] uppercase tracking-[0.16em] text-muted-foreground">Brand value {markupType === "fixed" ? "(₱)" : "(%)"}</span>
            ...
            <label className="mt-4 flex min-h-11 items-center gap-2 border-t border-border pt-3 text-[10px] text-muted-foreground">
          ...
          <section className="border border-[#9f7c3f] bg-[#17140F] p-5 text-[#FAF7F0]">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[9px] uppercase tracking-[0.2em] text-[#9F927E]">Price summary</p>
            </div>
            ...
            <div className="mt-4 border-t border-white/15 pt-4">
              <p className="text-[8px] uppercase tracking-[0.18em] text-[#9F927E]">Selling price</p>
              <p className="mt-1 font-serif text-3xl">{formatCentavos(totals.sellingPriceCentavos)}</p>
              <div className="mt-3 flex justify-between text-[10px] text-[#B9AE9C]">
          ...
          {warnings.length > 0 && (
            <section className="border border-amber-200 bg-amber-50 p-3" aria-live="polite">
              <div className="flex gap-2">
                ...
                <div>
                  <p className="text-[10px] font-medium text-amber-900">Review before quoting</p>
                  {warnings.map((warning) => <p key={warning} className="mt-1 text-[9px] text-amber-800">{warning}</p>)}
                </div>
              </div>
            </section>
          )}
